...
| Dockerfile (binary downloads) | mise itself (GPG-signed checksums), Cursor agent, opencode, Claude Code, codex (OpenAI) | `.devcontainer/Dockerfile` (ARG + RUN) |
...
- **codex CLI (OpenAI)**: Installed as a pinned + SHA256-verified + release-age-gated download of the `openai/codex` "package" release asset (`codex-package-<triple>.tar.gz` from tag `rust-vX.Y.Z`), mirroring the Cursor/opencode blocks. The native installer (`chatgpt.com/codex/install.sh`) lays its standalone payload under `~/.codex/packages/standalone`, but we deliberately extract to `~/.local/share/codex/<version>-<triple>/` (like Claude → `~/.local/share/claude`) and symlink `~/.local/bin/codex` at it. This is because **all of codex's state — `auth.json`, `config.toml`, sqlite session/state DBs, `skills/`, `cache/` — lives under `~/.codex` (CODEX_HOME)**, which is bind-mounted (`devcontainer.json` ← host `~/.codex`, source created by `init.sh`) so login/config/sessions persist across rebuilds. Baking the binary *inside* `~/.codex` would let that bind mount shadow it at runtime (same trap as the code-server data dir). `~/.codex` is the only directory codex uses — there is no `~/.config/codex`, `~/.cache/codex`, or `~/.local/share/codex` to mount. Renovate bumps `CODEX_VERSION` via `extractVersion=^rust-v(?<version>.+)$`; per-arch SHA256s are re-captured by hand from the release's `codex-package_SHA256SUMS` on each bump.
