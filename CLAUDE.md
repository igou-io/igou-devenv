...
├── code-server-sync.sh  # Bridges devcontainer.json customizations (settings + extensions) into code-server
...
├── tmux.conf            # tmux config (resurrect/continuum) copied to ~/.tmux.conf
...
|---|---|---|
| Dockerfile (dnf) | python3, podman, buildah, skopeo, jq, 1Password CLI, qemu-kvm, qemu-img, genisoimage, edk2-ovmf (virtualization), etc. | `.devcontainer/Dockerfile` |
...
- **Always run via privileged Docker on the host**: the devcontainer is launched by Docker — Cursor's Dev Containers extension and the `devcontainer` CLI both call `docker`. Podman is used only *inside* the container for nested work, never to run the devcontainer itself.
...
- **code-server (browser IDE, always-on)**: Installed via mise's `http:` backend (`mise.toml` → `http:code-server`). Upstream ships no checksums/signatures/SLSA provenance and is not in aqua-registry, so the blake3 TOFU pin in `mise.lock` is the verification floor (same as virtctl/kube-burner; audited as `blake3` in `tests/mise-expected-verification.toml`). Unlike other http: tools it's a wrapper+lib tree, so `bin_path = "bin"` + `strip_components = 1` point mise at the launcher. `post-start.sh` starts it always-on, bound to `0.0.0.0:8080` (reachable on the host via `--network=host`), with mandatory `auth: password`. Because the container is `--privileged` with `/dev` bind-mounted, the port grants full host access — auth must never be disabled; prefer SSH/Tailscale to reach it. Config ships in `dotfiles/code-server-config.yaml` (no secret), copied by `post-create.sh` **only if absent**; the password is generated on first start (override with the `PASSWORD` env var). **TLS**: `cert: true` serves HTTPS with a self-signed cert that code-server generates on first start via the `openssl` CLI (baked into the image — `cert: true` shells out to it) and writes into the persistent `~/.local/share/code-server` data dir; no private key is committed. `make run` passes `--cert` for the same on the published-image path. The self-signed cert is localhost-scoped, so browsing by IP shows the usual warning; HTTPS stops the password going out in cleartext but does not shrink the privileged-port blast radius (prefer SSH/Tailscale + localhost bind for real hardening). `~/.config/code-server` and `~/.local/share/code-server` are bind-mounted (sources created by `init.sh`), so the config/password, extensions, and settings persist across rebuilds — which is why the seed copy is conditional (it must not clobber the persisted config). Extensions come from Open VSX, not the MS marketplace. Defined in `mise.toml` and started by `.devcontainer/post-start.sh`.
- **code-server honors devcontainer.json customizations**: code-server does **not** read `devcontainer.json` — `customizations.vscode.settings`/`.extensions` are applied only by the VS Code Dev Containers *client*, and this repo launches code-server directly. `.devcontainer/code-server-sync.sh` bridges the gap (single source of truth: `devcontainer.json`): it writes `customizations.vscode.settings` to code-server's **Machine** scope (`~/.local/share/code-server/Machine/settings.json` — the devcontainer-owned layer; the user's `User/settings.json` theme is left intact) and installs `customizations.vscode.extensions` from Open VSX (per-extension failures, e.g. `anysphere.cursorpyright` which has no Open VSX build, warn but don't fail). It runs from **two** places because the data dir is bind-mounted (so an image-baked copy is shadowed at runtime for `make up`): the **Dockerfile** bakes it into the image's default data dir for `make run` (which has no lifecycle hooks), and **`post-start.sh`** re-runs it at start to populate the bind-mounted data dir for `make up`. Idempotent — already-installed extensions are skipped, so warm starts don't hit the network.
- **GitHub HTTPS+token auth (per-org)**: GitHub token auth is HTTPS-only, so the host `~/.gitconfig` (bind-mounted read-only, shared host+container) must **not** rewrite `https://github.com/...` to SSH — i.e. it carries no `[url "git@github.com:"] insteadOf = https://github.com/`. Instead it wires a github.com-scoped credential helper: `[credential "https://github.com"] helper = !~/bin/git-credential-gh-org` with `useHttpPath = true`. `~/bin/git-credential-gh-org` resolves in both environments — on the host `~/bin` mirrors the repo's `bin/`, and in the container `post-create.sh` symlinks `~/bin` → the repo `bin/`. The helper selects a token per org from the request path: `github.com/<org>/...` → `$GH_TOKEN_<ORG>` (upper-cased, non-alphanumerics → `_`, e.g. `igou-io` → `GH_TOKEN_IGOU_IO`, `david-igou` → `GH_TOKEN_DAVID_IGOU`), falling back to `$GH_TOKEN` then `$GITHUB_TOKEN`. Tokens are read live from the env at call time (never written to disk; `store`/`erase` are no-ops), so `use <env>` exporting `GH_TOKEN`/`GH_TOKEN_<ORG>` makes `git push`/`pull` work immediately and switching tokens is just re-exporting. Remotes should be clean `https://github.com/<org>/<repo>.git` (no baked-in token). SSH for `git@github.com:` remotes still works via the forwarded agent; only the implicit https→ssh rewrite is gone. (The `~/.gitconfig` itself is the user's host dotfile, not tracked in this repo — keep it in whatever provisions the host, e.g. `ansible-collection-devhost`.)
- **Terminal persistence (tmux)**: interactive code-server terminals auto-attach to a persistent `tmux` session `main` (via `dotfiles/.bashrc`, scoped to `TERM_PROGRAM=vscode` so `make shell`/CI/automation are untouched; `NO_AUTO_TMUX=1` opts out), so terminals survive code-server restarts and browser reconnects — open extra terminals as tmux windows (`Ctrl-b c`), not editor tabs. `tmux-resurrect` + `tmux-continuum` (baked into the image at `/opt/tmux-plugins`, pinned by commit SHA — tmux-resurrect cuts no releases) auto-save/restore the layout across `make rebuild`, saving to `~/.local/share/igou-devenv` (a bind mount). That same bind mount persists **bash history** across rebuilds (`HISTFILE` points there) and `history -a; history -n` in `PROMPT_COMMAND` shares history live across all open terminals. Config in `dotfiles/tmux.conf` (copied to `~/.tmux.conf` by `post-create.sh`); mount created by `init.sh`.
