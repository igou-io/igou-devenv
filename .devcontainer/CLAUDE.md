...
- Scoped sessions: if `AGENT_PROFILE` is set you are inside `agent-sandbox-launch` (adr/0006) with a fixed credential scope — `oc`/`kubectl` already work for that identity; there is no `use`, `ssh-use` or `op`. Do not work around a 403; say which profile a new session needs.
