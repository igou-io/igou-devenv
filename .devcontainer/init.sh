...
for dir in "$HOME/.claude-container" "$HOME/.codex" "$HOME/.config/cursor" "$HOME/.config/opencode" "$HOME/.local/share/opencode" "$HOME/.t3" "$HOME/.config/code-server" "$HOME/.local/share/code-server" "$HOME/.local/share/igou-devenv" "$HOME/.ssh" "$HOME/.kube" "$HOME/.config/argocd" "$HOME/.config/op" "$HOME/.terraform.d" "$HOME/workspace"; do
    if [ ! -d "$dir" ]; then
        mkdir -p "$dir"
