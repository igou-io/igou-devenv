...
for dir in "$HOME/.claude" "$HOME/.claude-container" "$HOME/.ssh" "$HOME/.kube" "$HOME/.config/argocd" "$HOME/.config/op" "$HOME/.terraform.d" "$HOME/workspace" "$HOME/rosa-gitops" "$HOME/rosa-gitops-example-team"; do
    if [ ! -d "$dir" ]; then
        mkdir -p "$dir"
